...
export function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#f8f1ec]">
      <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-white/70 bg-white/80 p-6 backdrop-blur lg:block">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-10 border-b border-white/70 bg-[#f8f1ec]/90 backdrop-blur">
          <div className="flex min-h-20 flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-8">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-cocoa">Dashboard</p>
              <h1 className="text-xl font-semibold text-ink">Welcome back{user ? `, ${user.displayName}` : ""}</h1>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Link to="/" className="btn-secondary">Public site</Link>
